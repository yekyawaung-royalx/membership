							...
							<div class="row">
				                ...
				                <div class="col-md-8">
									<div class="pull-right">
										<button type="button" class="btn btn-primary v-top btn-sync1" data-bs-toggle="modal" data-bs-target="#SyncModal" cursorshover="true" >
											<span class="tf-icons bx bx-sync" cursorshover="true"></span> Sync System Data
										</button>
					                    ...
					                    <table class="table">
					                        <thead>
        					                    <tr>
        					                        ...
        					                        <th class="text-muted unset">Ref/Digital ID</th>
						...
						if(data.total > 0){
		                    $.each(data.data, function (i, item) {
		                       	$("#fetched-data").append('<tr>'
									+'<td><span class="text-muted"><span class="fw-semibold">'+item.registered_at.substring(0, 10)+'</span><br>'+item.registered_at.substring(10, 19)+'</span></td>'
		                            +'<td><span class="fs-16 fw-bold '+(item.active == 1 ? '':'inactive')+' '+(item.active == 1 ? 'text-primary':'text-danger')+'">'+(item.ref != '' ? item.ref:'...')+'</span><br>'+item.digital_id+'</td>'
		                            +'<td class="text-primary '+(item.active == 1 ? '':'inactive')+'"><img src="'+url+'/_backend/_images/card.png" class="avatar v-base me-1"><span class="member-name">'+item.name+'<br>'+item.mobile+'</span></td>'
		                            +'<td class="'+(item.active == 1 ? '':'inactive')+'"><div class="featured-badge"><span class="lvl '+(item.user_level==1? '':'bg-gold')+'">'+item.user_level+'</span></div></td>'
		                            +'<td class="'+(item.active == 1 ? '':'inactive')+'">'+item.mobile+'</td>'
		                            +'<td class="'+(item.active == 1 ? '':'inactive')+'">'+member_status(item.status)+'</td>'
		                            +'<td>'
		                                +'<div class="btn-group">'
